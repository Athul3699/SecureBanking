...
class ManageRequestsTier1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      accounts: [],
  ...
  onButtonClick = (type, data) => {
    if (type == "approve") {
      postRequest(
        `${API_URL}/api/v1/transaction/ApproveMoneyTransferNonCritical`,
        { id: data.id }
      )
        .then(res => {
          this.refreshAccountsState();
      ...
      postRequest(
        `${API_URL}/api/v1/transaction/DeclineMoneyTransferNonCritical`,
        { id: data.id }
      )
        .then(res => {
          this.refreshAccountsState();
...
export default ManageRequestsTier1;
